...
client.once('ready', () => {
	...
	function updateStatus() {
		statusURL = `https://api.mozambiquehe.re/servers?auth=${config.api.Mozambique}`;
		...
		axios.all([status]).then(
			axios.spread((...responses) => {
				...
				try {
					var channelName = client.channels.cache.get(config.autoUpdate.status.channel);
					channelName.setName(`${statusEmoji}-game-status`);

					console.log(
						chalk`{blueBright [${DateTime.local().toFormat('hh:mm:ss')}] Updated Channel Status Indicator}`,
					);
				} catch (err) {
