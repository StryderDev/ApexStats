...
module.exports = {
	...
	execute(client) {
		...
		function updateStatus() {
			got.get(`https://api.mozambiquehe.re/servers?auth=${config.api.mozambique}`, { responseType: 'json' }).then(
				res => {
		...
		if (rotations.status.enabled == true) updateStatus();

		setInterval(function () {
			if (rotations.status.enabled == true) {
				var date = new Date();

				if (date.getMinutes() % rotations.status.interval == 0) {
					updateStatus();
				}
			}
