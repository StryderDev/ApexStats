...
module.exports = {
	data: new SlashCommandBuilder().setName('gun-run').setDescription('Shows the current Gun Run map.'),
	async execute(interaction) {
		const loadingEmbed = new EmbedBuilder().setDescription(`${Misc.Loading} Loading current Gun Run map...`).setColor('2F3136');
		...
		await axios
			...
			.then(response => {
				...
				const mapEmbed = new EmbedBuilder()
					.setTitle(`Legends are currently Running & Gunning in **${ltm.current.map}**.`)
					.setDescription(
					...
					.setColor('2F3136')
					.setImage(`https://cdn.jumpmaster.xyz/Bot/Maps/Season%2014/Gun%20Run/${encodeURIComponent(ltm.current.map)}.png`);
