...
module.exports = {
	...
	async execute(interaction) {
		const loadingEmbed = new EmbedBuilder().setDescription(`${Misc.Loading} Loading current in-game Control map...`).setColor('2F3136');

		await interaction.editReply({ embeds: [loadingEmbed] });

		function mapImage(map) {
			if (map == 'Hammond Labs After Dark') return 'Hammond Labs';
		}
		...
		await axios
			...
			.then(response => {
				const control = response.data.ltm;
				...
				const mapEmbed = new EmbedBuilder()
					...
					.setColor('2F3136')
					.setImage(`https://cdn.jumpmaster.xyz/Bot/Maps/Season%2014/Control/${encodeURIComponent(control.current.map)}.png`);
