					...
					axios.spread((brMap, rankedMap) => {
						...
						db.query(getMapIndex, (err, result) => {
							...
							if (result[0].brMapIndex != brData.rotationIndex) {
								client.user.setActivity(`${brData.map} / ${rankedData.map}`, { type: ActivityType.Custom });

								console.log(chalk.green(`${chalk.bold('[PRESENCE]')} Updated bot presence, set to "${brData.map} / ${rankedData.map}"`));
