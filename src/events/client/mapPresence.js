	...
	execute(client) {
		(async function mapPresenceLoop() {
			...
			if (currentMinute % process.env.PRESENCE_UPDATE == 0) {
				...
				console.log(`${chalk.yellow.bold('[APEXSTATS_PRESENCE]')} Checking map index...`);
					...
					.then(
						axios.spread((brMap, rankedMap) => {
							...
							db.query(getMapIndex, (err, result) => {
								if (err) return console.log(`${chalk.red.bold('[APEXSTATS_PRESENCE]')} Query error: ${err.code}`);
								...
								if (result[0].brMapIndex != brData.rotationIndex) {
									...
									console.log(`${chalk.green.bold('[APEXSTATS_PRESENCE]')} Updated bot presence, set to "${brData.map.name} / ${rankedData.map.name}"`);
									...
									db.query(updateMapIndex, (err, result) => {
										if (err) return console.error(`${chalk.red.bold('[APEXSTATS_PRESENCE]')} Query error: ${err.code}`);

										console.log(`${chalk.green.bold('[APEXSTATS_PRESENCE]')} Updated map index to ${brData.rotationIndex}`);
									});
								} else {
									console.log(`${chalk.green.bold('[APEXSTATS_PRESENCE]')} Map index is the same as current index`);
								}
							});
						}),
					)
					.catch(err => {
						console.error(`${chalk.red.bold('[APEXSTATS_PRESENCE]')} Axios error: ${err}`);
