...
function battlepass(data) {
	if (data.history['Marked_1'] >= 60) {
		rewardLevel = 60;
		rewardCompletion = 100;
	} else {
		rewardLevel = data.history['Marked_1'];
		rewardCompletion = Math.floor((data.history['Marked_1'] / 60) * 100);
	}

	if (data.history['Marked_1'] >= 100) {
		badgeLevel = 100;
		badgeCompletion = 100;
	} else {
		badgeLevel = data.history['Marked_1'];
		badgeCompletion = Math.floor((data.history['Marked_1'] / 100) * 100);
	}
