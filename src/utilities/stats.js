...
function battlepass(data) {
	if (data.history['Overclocked_2'] >= 60) {
		rewardLevel = 60;
		rewardCompletion = 100;
	} else {
		rewardLevel = data.history['Overclocked_2'];
		rewardCompletion = Math.floor((data.history['Overclocked_2'] / 60) * 100);
	}

	if (data.history['Overclocked_2'] >= 100) {
		badgeLevel = 100;
		badgeCompletion = 100;
	} else {
		badgeLevel = data.history['Overclocked_2'];
		badgeCompletion = Math.floor((data.history['Overclocked_2'] / 100) * 100);
	}
