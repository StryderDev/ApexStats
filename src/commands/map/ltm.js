...
module.exports = {
	...
	async execute(interaction) {
		...
		const loadingEmbed = new EmbedBuilder().setDescription(`${emotes.loading} Loading LTM map data...`);
		...
		await axios
			...
			.then(async res => {
				...
				if (map.active == false) {
					const mapEmbed = new EmbedBuilder().setTitle(`${emotes.offline} No Active Map`).setDescription(`${emotes.listArrow} There is no active LTM map in rotation.`);
					...
					const mapEmbed = new EmbedBuilder()
						...
						.setDescription(
							`${emotes.listArrow} **${mapInfo.name}** ends <t:${map.times.nextMap}:R> at <t:${map.times.nextMap}:t>\n${emotes.listArrow} Up Next: **${map.next[0].map.type} - ${
								map.next[0].map.name
							}** for ${nextMapLength(map.next[0].duration)}`,
						)
					...
					const mapEmbed = new EmbedBuilder()
						.setTitle(`${emotes.online} ${nextAmount} Next LTM Map Rotations`)
