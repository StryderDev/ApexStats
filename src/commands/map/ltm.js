...
module.exports = {
	data: new SlashCommandBuilder()
		.setName('ltm')
		.setDescription('View current and future LTM map rotations')
	...
	async execute(interaction) {
		...
		const loadingContainer = new ContainerBuilder().addTextDisplayComponents(textDisplay => textDisplay.setContent(`${emotes.loading} Loading LTM map data...`));
		...
		await axios
			.get(`https://solaris.apexstats.dev/beacon/map/ltm?key=${process.env.SPYGLASS}&next=${nextAmount}`)
			.then(async res => {
				const map = res.data;
				const mapInfo = map.map;

				if (map.active == false) {
					const inactiveContainer = new ContainerBuilder().addTextDisplayComponents(textDisplay => textDisplay.setContent(`# LTM Rotation Disabled\n${emotes.listArrow} There is no active LTM map in rotation`));
					...
					const singleMapContainer = new ContainerBuilder()
						.addTextDisplayComponents(textDisplay =>
							textDisplay.setContent(
								`# ${emotes.online} ${mapInfo.type} - ${mapInfo.name}\n${emotes.listArrow} **${mapInfo.type} - ${mapInfo.name}** ends <t:${map.times.nextMap}:R> at <t:${map.times.nextMap}:t> ${mapNextString}`,
							),
						)
						.addMediaGalleryComponents(mediaGallery =>
							mediaGallery.addItems(mediaGalleryItem =>
								mediaGalleryItem.setDescription('Map Image for the LTM Mode').setURL(`https://specter.apexstats.dev/ApexStats/Maps/${mapImage}.png?key=${process.env.SPECTER}`),
							),
						);
				...
				const nextMapContainer = new ContainerBuilder().addTextDisplayComponents(textDisplay => textDisplay.setContent(`# Upcoming LTM Maps\n${nextMapString}`));
