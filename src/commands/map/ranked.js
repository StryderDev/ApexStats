...
module.exports = {
	...
	async execute(interaction) {
		...
		await axios
			.get(`https://api.jumpmaster.xyz/map/?next=1`)
			.then(response => {
				...
				const mapEmbed = new EmbedBuilder()
					...
					.setDescription(
						`**${map.map.name}** ends <t:${map.times.next}:R> at <t:${map.times.next}:t>.\n**Next Up:** ${map[0].map.type} - ${map.next[0].map.name} for 24 hours.`,
					)
					.setImage(`https://cdn.jumpmaster.xyz/Bot/Maps/Season%2016/Battle%20Royale/${encodeURIComponent(map.map.name)}.png?t=${Math.floor(Math.random() * 10)}`)
					.setColor(embedColor);
