...
module.exports = {
	...
	async execute(interaction) {
		...
		const loadingContainer = new ContainerBuilder().addTextDisplayComponents(textDisplay => textDisplay.setContent(`${emotes.loading} Loading Mixtape map data...`));
			...
			.then(async res => {
				...
				if (map.active == false) {
					const inactiveContainer = new ContainerBuilder().addTextDisplayComponents(textDisplay =>
						textDisplay.setContent(`# Mixtape Rotation Disabled\n${emotes.listArrow} There is no active Mixtape map in rotation`),
					);
					...
					const singleMapContainer = new ContainerBuilder()
						...
						.addMediaGalleryComponents(mediaGallery =>
							mediaGallery.addItems(mediaGalleryItem =>
								mediaGalleryItem.setDescription('Map Image for the Mixtape Mode').setURL(`https://specter.apexstats.dev/ApexStats/Maps/${mapImage}.png?key=${process.env.SPECTER}`),
							),
						);
				...
				const nextMapContainer = new ContainerBuilder().addTextDisplayComponents(textDisplay => textDisplay.setContent(`# Upcoming Mixtape Maps\n${nextMapString}`));
