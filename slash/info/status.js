...
module.exports = {
	...
	run: async (client, interaction) => {
		...
		got.get(`https://api.mozambiquehe.re/servers?auth=${config.api.mozambique}`, { responseType: 'json' })
			.then(res => {
				const data = JSON.parse(res.body);

				console.log(chalk`{yellow ${time} Status Command Response Code: ${res.statusCode}}`);
				...
				interaction.followUp({ embeds: [statusEmbed] }).catch(err => {
					console.log(err);
